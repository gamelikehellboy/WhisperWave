...
async def websocket_endpoint(websocket: WebSocket):
    ...
    while True:
        try:
            ...
            await asyncio.sleep(0.1)  # Adjust this for frame rate control
...
async def get():
    ...
    <html>
    <body>
    <h1>WebSocket Video Feed</h1>
    <script>
      const ws = new WebSocket('ws://127.0.0.1:8000/ws/video_feed');
      ws.onmessage = (event) => {
        const data = JSON.parse(event.data);
        if (data.frame) {
          document.querySelector('img').src = 'data:image/jpeg;base64,' + data.frame;
          document.querySelector('p').innerText = 'Converted Text: ' + data.word;
        }
      };
    ...
    <img src="" alt="Video Stream" style="width: 50%; border: 2px solid black; border-radius: 10px;">
    <p>Converted Text:</p>
    </body>
    </html>
    """)
